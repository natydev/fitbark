RSpec.describe Fitbark::Client do
  ...
  let!(:activity_series_hourly_response) { json_response(:activity_series_hourly) }
  let!(:as_params) do
    {
      dog_slug: 'abc',
  ...
  context 'activity_series (hourly)' do
    context 'when response is success' do
      let!(:stub_webmock_success) do
        stub_request(:post, "#{api_uri_request}/activity_series")
          ...
          .to_return(status: 200, body:
            activity_series_hourly_response, headers: {})
      end
      ...
      it 'each item inside array is kind Fitbark::Data::ActivityHourly' do
        expect(subject.activity_series(as_params).first)
          .to be_kind_of(Fitbark::Data::ActivityHourly)
      end
    end
